...
class _YouTubeScreenState extends State<YouTubeScreen> {
  ...
  // 热门视频数据
  final List<YouTubeVideo> _popularVideos = [];
  bool _isLoadingPopular = false;

  // 搜索数据
  final List<YouTubeVideo> _searchResults = [];
  ...
  Future<void> _fetchPopularVideos() async {
    ...
    final result = await YouTubeService.getPopular();
  ...
  Future<void> _performSearch(String query) async {
    ...
    final result = await YouTubeService.search(query.trim());
  ...
  /// 打开外部浏览器播放 YouTube 视频
  Future<void> _launchYouTubeVideo(YouTubeVideo video) async {
            ...
            slivers: [
              ...
              if (_hasSearched) ...[
                ...
                if (_isLoadingPopular && _popularVideos.isEmpty)
                  ...
                  SliverToBoxAdapter(
                    child: _buildPopularVideoList(themeService),
                  ),
                ],
              ],
  ...
  /// 构建热门视频横向滚动卡片
  Widget _buildPopularVideoList(ThemeService themeService) {
    return SizedBox(
      height: 220,
      child: ListView.separated(
        ...
        itemBuilder: (context, index) {
          ...
          return _buildPopularVideoCard(video, themeService);
        },
      ),
    );
  }
  ...
  /// 构建单个热门视频卡片
  Widget _buildPopularVideoCard(YouTubeVideo video, ThemeService themeService) {
            ...
            Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    '${_formatViewCount(video.viewCount)} 次观看',
                    style: FontUtils.poppins(context,
                      fontSize: 11,
                      color: themeService.isDarkMode
                          ? const Color(0xFFb0b0b0)
  ...
  /// 构建搜索结果列表项
  Widget _buildSearchResultItem(YouTubeVideo video, ThemeService themeService) {
    return GestureDetector(
      onTap: () => _launchYouTubeVideo(video),
      child: Container(
