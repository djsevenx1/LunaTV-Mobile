...
| 视频功能 | 完整 | 跨源搜索 / 详情 / SSE 流式搜 / 源浏览器 / 番剧日历 / 豆瓣 / 网盘,短剧 / B站 / YouTube 占位 |
| 额外能力 | — | 每账号 18+ 源标记 + 过滤开关 (v0.8) |
...
每个视频源可在 Web UI 标记为「🔞 18+」(默认全部安全源)。Web UI 顶部有「仅本账号」开关,默认关闭 → 该账号 app 看不到 18+ 源;开启 → 该账号 app 看到所有源,App 根据 `adult` 字段显示标签。账号之间互不影响。
